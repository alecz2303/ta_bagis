...
class OrderRepository extends Repository
{
    ...
    public function createOrderIfNotThenRetry(array $data)
    {
        ...
        try {
            ...
            $order = $this->model->create(array_merge($data, [
            ...
            foreach ($data['items'] as $item) {
                ...
                $orderItem = $this->orderItemRepository->create(array_merge($item, [
                    'order_id' => $order->id,
                ]));

                if (!empty($item['children'])) {
                    foreach ($item['children'] as $child) {
                        $this->orderItemRepository->create(array_merge($child, [
                            'order_id' => $order->id,
            ...
            // 👇 IMPORTANTÍSIMO: no reintentar infinito, mejor re-lanzar
            throw $e;
        }
    }
